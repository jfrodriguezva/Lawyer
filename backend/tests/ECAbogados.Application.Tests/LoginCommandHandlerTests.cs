...
public class LoginCommandHandlerTests
{
    ...
    public async Task Credenciales_correctas_devuelve_token()
    {
        ...
        usuarios.Seed(new Usuario { Email = "erika@ecgabogados.com", PasswordHash = "hashed:Cambiar123!", Nombre = "Erika", Rol = "Administrador", Activo = true });

        var resultado = await handler.Handle(new LoginCommand("erika@ecgabogados.com", "Cambiar123!"), CancellationToken.None);

        Assert.Equal("token-for-erika@ecgabogados.com", resultado.Token);
    ...
    public async Task Password_incorrecto_lanza_UnauthorizedAccessException()
    {
        ...
        await Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            handler.Handle(new LoginCommand("erika@ecgabogados.com", "incorrecta"), CancellationToken.None));
    }
    ...
    {
        var (handler, usuarios) = CrearHandler();
        usuarios.Seed(new Usuario { Email = "asistente@ecgabogados.com", PasswordHash = "hashed:Asistente123!", Nombre = "Asistente", Rol = "Asistente", Activo = false });

        await Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            handler.Handle(new LoginCommand("asistente@ecgabogados.com", "Asistente123!"), CancellationToken.None));
    }
    ...
    public async Task Quinto_intento_fallido_bloquea_la_cuenta_incluso_con_password_correcta()
    {
        ...
        var ex = await Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            handler.Handle(new LoginCommand("erika@ecgabogados.com", "Cambiar123!"), CancellationToken.None));
