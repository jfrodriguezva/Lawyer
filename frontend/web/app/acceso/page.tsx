...
import { login, loginCliente, olvidePassword, ApiError } from "@/lib/api";
import { setCookie } from "@/lib/cookies";

// Acceso único: no se le pide al usuario que elija entre "personal" o
// "cliente". Probamos primero el login de personal y, si el correo no
// corresponde a una cuenta de personal (401), probamos el de clientes.
// Quien resulte autenticado se redirige a su portal correspondiente.
export default function AccesoPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const res = await loginCliente(email, password);
      setCookie("ecg_cliente_token", res.token);
      setCookie("ecg_cliente_user", JSON.stringify({ nombre: res.nombre, email: res.email }));
