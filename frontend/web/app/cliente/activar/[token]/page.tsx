...
import { aceptarInvitacionCliente, ApiError } from "@/lib/api";

export default function ActivarClientePage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      await aceptarInvitacionCliente(params.token, nuevaPassword);
      setListo(true);
    } catch (err) {
      ...
      <div className="w-full max-w-md border border-brand-line bg-brand-ink2 px-10 py-12">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-2 text-sm text-brand-creamSoft">
            Elige una contraseña para consultar tus casos, documentos y citas.
          </p>
        </div>
        ...
        {listo ? (
          ...
          <form onSubmit={handleSubmit} className="mt-10 space-y-5">
            ...
            >
              {loading ? "Guardando…" : "Activar mi acceso"}
            </button>
          </form>
        )}
      </div>
